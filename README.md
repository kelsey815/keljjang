...
| 왓챠 | 왓챠 홈 "왓챠 TOP 20" 섹션 (Playwright) | 영화·시리즈 혼합 20편 · `/contents/m` = 영화, `/t` = 시리즈 |
| 쿠팡플레이 | `data/coupang_manual.csv` (수동 입력) | 공식 웹이 Akamai Bot Manager로 크롤러 차단돼서 사용자가 주 1회 직접 페이지의 "지금 인기있는 콘텐츠" TOP 20을 붙여넣어 갱신 |
| 티빙 | 키노라이츠 필터링 랭킹 | 비로그인 홈이 온보딩으로 강제 리디렉션 |

**네이버 영화 메타(개봉일·관객수·감독·장르)** 추출:
- 영화: 검색 결과 `.fds-infolist` 인포박스에서 키-값 페어 파싱 (배우 ↔ 감독 혼선 방지)
- 시리즈: "제목 드라마" / "제목 예능" 쿼리로 네이버 카드의 장르·연출 추출
